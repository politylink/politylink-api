...
def get_bills_api():
    kwargs = {
        ...
        'submitted_groups': request.args.getlist('sbgroup', lambda x: int(x)),
        'supported_groups': request.args.getlist('spgroup', lambda x: int(x)),
        'opposed_groups': request.args.getlist('opgroup', lambda x: int(x)),
